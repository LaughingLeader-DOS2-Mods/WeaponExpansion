package LS_Classes
{
	...
	public class scrollbar extends MovieClip
	{
		...
		public function scrollbar(param1:String = "down_id", param2:String = "up_id", param3:String = "handle_id", param4:String = "scrollBg_id", param5:String = "", param6:String = "")
		{
			...
			var val14:Class = null;
			this.m_scrollWheelTimer = new Timer(100);
			super();
			this.m_tweenY = 0;
			var val7:Class = param1 == ""?MovieClip:getDefinitionByName(param1) as Class;
			var val8:Class = param2 == ""?MovieClip:getDefinitionByName(param2) as Class;
			...
			var val11:Sprite = new Sprite();
			val11.graphics.lineStyle(0,16777215);
			val11.graphics.beginFill(16777215);
			val11.graphics.drawRect(0,0,10,10);
			val11.graphics.endFill();
			this.m_frameHit_mc.addChild(val11);
			val11.alpha = 0;
			addChild(this.m_frameHit_mc);
			addChild(this.m_handle_mc);
			addChild(this.m_up_mc);
			addChild(this.m_down_mc);
			var val12:Number = this.m_bg_mc.width;
			if(val12 < this.m_up_mc.width)
			{
				val12 = this.m_up_mc.width;
			}
			if(val12 < this.m_down_mc.width)
			{
				val12 = this.m_down_mc.width;
			}
			if(val12 < this.m_handle_mc.width)
			{
				val12 = this.m_handle_mc.width;
			}
			this.m_bg_mc.x = Math.round((val12 - this.m_bg_mc.width) * 0.5);
			this.m_up_mc.x = Math.round((val12 - this.m_up_mc.width) * 0.5);
			this.m_down_mc.x = Math.round((val12 - this.m_down_mc.width) * 0.5);
			this.m_handle_mc.x = Math.round((val12 - this.m_handle_mc.width) * 0.5);
			this.m_frameHit_mc.width = val12;
			if(param5 != "")
			{
				val13 = getDefinitionByName(param5) as Class;
				this.m_FFdown_mc = new val13();
				...
				this.m_FFdown_mc.x = Math.round((val12 - this.m_FFdown_mc.width) * 0.5);
				this.m_ffDownH = this.m_FFdown_mc.height;
			}
			if(param6 != "")
			{
				val14 = getDefinitionByName(param6) as Class;
				this.m_FFup_mc = new val14();
				...
				this.m_FFup_mc.x = Math.round((val12 - this.m_FFup_mc.width) * 0.5);
